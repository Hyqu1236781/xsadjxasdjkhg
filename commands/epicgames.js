...
module.exports.run = async (client, msg, args, config) => {
    if(cooldown.has(msg.author.id)) {
        msg.reply(`Musisz Poczekać ${config.COOLDOWN} Minut aby wygenerować kolejne konto!`)
            .then((m) => {
                msg.delete();
        ...
        fs.readFile('./accounts/epic.txt', 'utf8', function(err, data) {
            if (err) throw err;
            ...
            fs.writeFile('./accounts/epic.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });

            let embed = new Discord.RichEmbed()
            .addField('Epic Games Accounts',`\n**${account}**`)
